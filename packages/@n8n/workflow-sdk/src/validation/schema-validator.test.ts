...
describe('schema-validator', () => {
	...
	describe('loadSchema', () => {
		...
		it('loads schema for flat version structure (e.g., set v2)', () => {
			...
			const schema = loadSchema('resin-nodes-base.set', 2);
		...
		it('loads schema for version with decimal (e.g., httpRequest v4.2)', () => {
			...
			const schema = loadSchema('resin-nodes-base.httpRequest', 4.2);
			expect(schema).not.toBeNull();
		});

		it('loads schema for langchain nodes with @n8n prefix', () => {
			// LcAgentV1ConfigSchema in nodes/n8n-nodes-langchain/agent/v1.schema.js
			const schema = loadSchema('@resin/n8n-nodes-langchain.agent', 1);
			expect(schema).not.toBeNull();
		});

		it('caches schema after first load', () => {
			// First load
			const schema1 = loadSchema('resin-nodes-base.set', 2);
			// Second load should return same cached instance
			const schema2 = loadSchema('resin-nodes-base.set', 2);
	...
	describe('validateNodeConfig', () => {
		...
		it('returns valid:true for valid config with correct parameter types', () => {
			...
			const result = validateNodeConfig('resin-nodes-base.set', 2, {
		...
		it('validates AI node with valid subnode config', () => {
			...
			const result = validateNodeConfig('@resin/n8n-nodes-langchain.agent', 1, {
				parameters: {
				...
				subnodes: {
					model: { type: '@resin/n8n-nodes-langchain.lmChatOpenAi', version: 1 },
				},
			});
			expect(result.valid).toBe(true);
		});
		...
		it('validates AI node subnode config with array of tools', () => {
			...
			const result = validateNodeConfig('@resin/n8n-nodes-langchain.agent', 1, {
				...
				subnodes: {
					model: { type: '@resin/n8n-nodes-langchain.lmChatOpenAi', version: 1 },
					tools: [
						{ type: '@resin/n8n-nodes-langchain.toolCode', version: 1 },
