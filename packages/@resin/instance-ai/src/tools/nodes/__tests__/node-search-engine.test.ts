...
describe('NodeSearchEngine', () => {
	...
	describe('searchByName', () => {
		...
		it('should include builder hint message when present', () => {
			...
			const agentResult = results.find((r) => r.name === '@resin/nodes-langchain.agent');
	...
	describe('searchByConnectionType', () => {
		...
		it('should apply name filter when provided', () => {
			...
			expect(results.length).toBeGreaterThan(0);
			expect(results[0].name).toBe('@resin/nodes-langchain.memoryBufferWindow');
		});
	...
	describe('getRelatedSubnodeIds', () => {
		it('should return related subnode IDs from builderHint.inputs', () => {
			const related = engine.getRelatedSubnodeIds(['@resin/nodes-langchain.agent'], new Set());
			expect(related.has('@resin/nodes-langchain.lmChatOpenAi')).toBe(true);
			expect(related.has('@resin/nodes-langchain.memoryBufferWindow')).toBe(true);
		});
		...
		it('should exclude IDs in the excludeNodeIds set', () => {
			...
			expect(related.has('@resin/nodes-langchain.lmChatOpenAi')).toBe(false);
			expect(related.has('@resin/nodes-langchain.memoryBufferWindow')).toBe(true);
		});
		...
		it('should not include the initial node IDs in the result', () => {
			const related = engine.getRelatedSubnodeIds(['@resin/nodes-langchain.agent'], new Set());
			expect(related.has('@resin/nodes-langchain.agent')).toBe(false);
		});
	});
	...
	describe('getSubnodesForConnectionType', () => {
		it('should return default subnodes for known connection types', () => {
			...
			expect(lmSubnodes).toEqual(['@resin/nodes-langchain.lmChatOpenAi']);
		});
