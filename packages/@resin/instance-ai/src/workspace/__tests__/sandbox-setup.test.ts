...
describe('PACKAGE_JSON', () => {
	...
	it('should include a registry SDK dependency when workspace SDK linking is disabled', () => {
		...
		expect(packageJson.dependencies['@resin/workflow-sdk']).toBeDefined();
	...
	it('should omit the registry SDK dependency when workspace SDK linking is enabled', () => {
		...
		expect(packageJson.dependencies).not.toHaveProperty('@resin/workflow-sdk');
		expect(packageJson.dependencies.tsx).toBeDefined();
	});
});
...
describe('formatNodeCatalogLine', () => {
	it('should format a basic node with a string version', () => {
		...
		expect(result).toBe(
			'resin-nodes-base.httpRequest | HTTP Request | Makes an HTTP request and returns the response data | v1',
		);
	});
	...
	it('should pick the last element when version is an array', () => {
		...
		expect(result).toBe('resin-nodes-base.slack | Slack | Send messages to Slack | v3');
	});
	...
	it('should append aliases when codex.alias is present and non-empty', () => {
		...
		expect(result).toBe(
			'resin-nodes-base.gmail | Gmail | Send and receive emails via Gmail | v2 | aliases: email, google mail',
		);
	});
	...
	it('should not append aliases when codex.alias is an empty array', () => {
		...
		expect(result).toBe('resin-nodes-base.set | Set | Sets values on items | v1');
	});

	it('should not append aliases when codex is present but alias is undefined', () => {
		const node: SearchableNodeDescription = {
			name: 'resin-nodes-base.noOp',
		...
		expect(result).toBe('resin-nodes-base.noOp | No Operation | Does nothing | v1');
	});
	...
	it('should handle pipe characters in description (documents current behavior)', () => {
		...
		expect(result).toBe(
			'resin-nodes-base.ifNode | IF | Route items based on true | false condition | v1',
		);
	...
	it('should handle a single-element version array', () => {
		...
		expect(result).toBe('resin-nodes-base.code | Code | Run custom JavaScript code | v2');
	});
});
