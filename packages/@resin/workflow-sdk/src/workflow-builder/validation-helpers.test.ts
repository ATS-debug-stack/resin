...
describe('workflow-builder/validation-helpers', () => {
	...
	describe('isToolNode', () => {
		it('returns true for types containing tool', () => {
			expect(isToolNode('@resin/nodes-langchain.toolCalculator')).toBe(true);
			expect(isToolNode('@resin/nodes-langchain.toolCode')).toBe(true);
		});

		it('returns true for types containing Tool', () => {
			expect(isToolNode('@resin/nodes-langchain.mcpClientTool')).toBe(true);
		});

		it('returns false for non-tool types', () => {
			expect(isToolNode('resin-nodes-base.httpRequest')).toBe(false);
			expect(isToolNode('@resin/nodes-langchain.agent')).toBe(false);
		});
	});

	describe('TOOLS_WITHOUT_PARAMETERS', () => {
		it('contains known tools without parameters', () => {
			expect(TOOLS_WITHOUT_PARAMETERS.has('@resin/nodes-langchain.toolCalculator')).toBe(true);
			expect(TOOLS_WITHOUT_PARAMETERS.has('@resin/nodes-langchain.toolWikipedia')).toBe(true);
		});
	});
