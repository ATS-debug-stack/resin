...
describe('generateSubnodeCall', () => {
	it('generates inline subnode call with type and version', () => {
		const node = createSemanticNode('Model', '@resin/nodes-langchain.lmChatOpenAi');
		...
		expect(result).toContain("type: '@resin/nodes-langchain.lmChatOpenAi'");
		expect(result).toContain('version: 1');
		expect(result).toContain('languageModel({');
	});

	it('includes name when different from default', () => {
		const node = createSemanticNode('MyCustomModel', '@resin/nodes-langchain.lmChatOpenAi');
		const graph: SemanticGraph = {
	...
	it('uses variable references when useVarRefs=true', () => {
		...
		const parentSubnode = createSemanticNode('Tool', '@resin/nodes-langchain.toolWorkflow', [
...
describe('generateSubnodesConfig', () => {
	it('generates config for single subnode', () => {
		const subnode = createSemanticNode('Model', '@resin/nodes-langchain.lmChatOpenAi');
		const parent = createSemanticNode('Agent', '@resin/nodes-langchain.agent', [
	...
	it('generates config for multiple tools as array', () => {
		const tool1 = createSemanticNode('Tool1', '@resin/nodes-langchain.toolWorkflow');
		const tool2 = createSemanticNode('Tool2', '@resin/nodes-langchain.toolWorkflow');
		const parent = createSemanticNode('Agent', '@resin/nodes-langchain.agent', [
