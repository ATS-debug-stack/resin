...
`;
			...
			const openAiNode = parsedJson.nodes.find(
				(n) => n.type === '@resin/nodes-langchain.lmChatOpenAi',
			);
			...
			const originalJson: WorkflowJSON = {
				...
				nodes: [
					...
					},
					{
						...
						type: '@resin/nodes-langchain.textClassifier',
						typeVersion: 1.2,
						position: [200, 0],
						parameters: {},
					},
...
const agent1 = node({
  ...
  config: {
    ...
    subnodes: {
      model: languageModel({ type: '@resin/nodes-langchain.lmChatOpenAi', version: 1.3, config: { name: 'Model 1', parameters: { model: { mode: 'list', value: 'gpt-4o-mini' } } } }),
...
const agent2 = node({
  ...
  config: {
    ...
    subnodes: {
      model: languageModel({ type: '@resin/nodes-langchain.lmChatOpenAi', version: 1.3, config: { name: 'Model 2', parameters: { model: { mode: 'list', value: 'gpt-4o-mini' } } } }),
      tools: [sharedCalculator]
    }
  }
});
...
`;
			...
			const toolNodes = parsedJson.nodes.filter(
				(n) => n.type === '@resin/nodes-langchain.toolCalculator',
			);
...
`;
			...
			const embeddingNodes = parsedJson.nodes.filter(
				(n) => n.type === '@resin/nodes-langchain.embeddingsOpenAi',
			);
...
`;
			...
			const agentNodes = parsedJson.nodes.filter(
				(n) => n.type === '@resin/nodes-langchain.agent',
			);
...
describe('Committed workflows — schema validation errors', () => {
	...
	const mockNodeTypesProvider = {
		getByNameAndVersion: (type: string, _version?: number) => {
			if (type === '@resin/nodes-langchain.chatTrigger') {
				return {
					description: {
						inputs: ['main'],
						builderHint: {
			...
			if (type === '@resin/nodes-langchain.agent') {
				return {
					description: {
						inputs: ['main'],
						builderHint: {
			...
			if (type === '@resin/nodes-langchain.vectorStoreInMemory') {
				return {
					description: {
						inputs: ['main'],
						builderHint: {
