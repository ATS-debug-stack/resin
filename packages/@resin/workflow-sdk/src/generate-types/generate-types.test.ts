  ...
  --chat--color--secondary: #20b69e;
	...
	describe('nodeNameToFileName', () => {
		it('should convert node name to valid file name', () => {
			...
			expect(generateTypes.nodeNameToFileName('@resin/nodes-langchain.lmChatOpenAi')).toBe(
				'lmChatOpenAi',
			);
		});
	});
	...
	describe('getPackageName', () => {
		it('should extract package name from node type', () => {
			...
			expect(generateTypes.getPackageName('@resin/nodes-langchain.agent')).toBe(
	...
	describe('subnode union type generation', () => {
		...
		const mockVectorStoreNode: NodeTypeDescription = {
			name: '@resin/nodes-langchain.vectorStorePinecone',
		...
		const mockTextSplitterNode: NodeTypeDescription = {
			name: '@resin/nodes-langchain.textSplitterRecursiveCharacterTextSplitter',
		...
		describe('groupNodesByOutputType', () => {
			it('should group nodes by their AI output types', () => {
				...
				expect(grouped.ai_languageModel).toContain('@resin/nodes-langchain.lmChatOpenAi');
				expect(grouped.ai_languageModel).toContain('@resin/nodes-langchain.lmChatAnthropic');
				expect(grouped.ai_languageModel?.length).toBe(2);

				// Tool nodes grouped together
				expect(grouped.ai_tool).toContain('@resin/nodes-langchain.toolCode');
				expect(grouped.ai_tool).toContain('@resin/nodes-langchain.toolCalculator');
				expect(grouped.ai_tool?.length).toBe(2);

				// Memory nodes
				expect(grouped.ai_memory).toContain('@resin/nodes-langchain.memoryBufferWindow');
				expect(grouped.ai_memory?.length).toBe(1);

				// Main output nodes NOT included in AI subnode groups
				expect(grouped.main).toContain('@resin/nodes-langchain.agent');
			});
		...
		describe('generateSubnodeUnionTypes', () => {
			it('should generate ValidLanguageModelType union', () => {
				...
				expect(code).toContain("'@resin/nodes-langchain.lmChatOpenAi'");
				expect(code).toContain("'@resin/nodes-langchain.lmChatAnthropic'");
				// Should NOT include agent (it outputs 'main', not 'ai_languageModel')
				expect(code).not.toContain("'@resin/nodes-langchain.agent'");
			});
			...
			it('should generate ValidToolType union', () => {
				...
				expect(code).toContain("'@resin/nodes-langchain.toolCode'");
				expect(code).toContain("'@resin/nodes-langchain.toolCalculator'");
			});
