...
export function nodeNameToFileName(nodeName: string): string {
	...
	// @resin/nodes-langchain.lmChatOpenAi -> lmChatOpenAi
...
export function getPackageName(nodeName: string): string {
	...
	// @resin/nodes-langchain.agent -> n8n-nodes-langchain
	const parts = nodeName.split('.');
	const pkg = parts[0];
	// Handle @resin/ prefix
...
export async function generateTypes(): Promise<void> {
	...
	if (fs.existsSync(NODES_LANGCHAIN_TYPES)) {
		...
		const nodesLangchain = await loadNodeTypes(NODES_LANGCHAIN_TYPES, '@resin/nodes-langchain');
