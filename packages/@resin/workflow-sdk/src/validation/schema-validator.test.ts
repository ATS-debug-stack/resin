...
describe('schema-validator', () => {
	...
	describe('loadSchema', () => {
		...
		it('loads schema for langchain nodes with @n8n prefix', () => {
			...
			const schema = loadSchema('@resin/nodes-langchain.agent', 1);
			expect(schema).not.toBeNull();
		});
	...
	describe('validateNodeConfig', () => {
		...
		it('validates AI node with valid subnode config', () => {
			...
			const result = validateNodeConfig('@resin/nodes-langchain.agent', 1, {
				parameters: {
				...
				subnodes: {
					model: { type: '@resin/nodes-langchain.lmChatOpenAi', version: 1 },
				},
			});
			expect(result.valid).toBe(true);
		});
		...
		it('validates AI node subnode config with array of tools', () => {
			...
			const result = validateNodeConfig('@resin/nodes-langchain.agent', 1, {
				...
				subnodes: {
					model: { type: '@resin/nodes-langchain.lmChatOpenAi', version: 1 },
					tools: [
						{ type: '@resin/nodes-langchain.toolCode', version: 1 },
