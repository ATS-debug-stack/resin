...
describe('identifyPinDataNodes', () => {
	...
	it('should include root AI nodes targeted by ai_* connections', () => {
		const workflow: SimpleWorkflow = {
			name: 'Test',
			nodes: [
				makeNode({ name: 'Chat Trigger', type: '@resin/nodes-langchain.chatTrigger' }),
				makeNode({ name: 'Agent', type: '@resin/nodes-langchain.agent' }),
				makeNode({ name: 'OpenAI', type: '@resin/nodes-langchain.lmOpenAi' }),
				makeNode({ name: 'Tool', type: '@resin/nodes-langchain.toolCalculator' }),
			],
			connections: {
				'Chat Trigger': {
