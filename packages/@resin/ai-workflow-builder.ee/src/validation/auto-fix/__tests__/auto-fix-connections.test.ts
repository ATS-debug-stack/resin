...
describe('autoFixConnections', () => {
	...
	describe('Pass 1: Fix nodes missing required inputs', () => {
		it('should auto-fix when exactly one sub-node can provide the missing input', () => {
			const workflow = createWorkflow(
				[
					...
					createNode({ name: 'AI Agent', type: '@resin/nodes-langchain.agent' }),
					createNode({ name: 'OpenAI Model', type: '@resin/nodes-langchain.lmChatOpenAi' }),
				],
				{
					'Manual Trigger': {
			...
			const violations: ProgrammaticViolation[] = [
				{
					...
					type: 'critical',
					description:
						'Node AI Agent (@resin/nodes-langchain.agent) is missing required input of type ai_languageModel',
					pointsDeducted: 50,
					metadata: {
						nodeName: 'AI Agent',
						nodeType: '@resin/nodes-langchain.agent',
				...
				},
				{
					...
					type: 'critical',
					description:
						'Sub-node OpenAI Model (@resin/nodes-langchain.lmChatOpenAi) provides ai_languageModel but is not connected to a root node.',
					pointsDeducted: 50,
					metadata: {
		...
		it('should report unfixable when multiple sub-nodes can provide the same input', () => {
			const workflow = createWorkflow([
				createNode({ name: 'AI Agent', type: '@resin/nodes-langchain.agent' }),
				createNode({ name: 'OpenAI Model 1', type: '@resin/nodes-langchain.lmChatOpenAi' }),
				createNode({ name: 'OpenAI Model 2', type: '@resin/nodes-langchain.lmChatOpenAi' }),
			]);
		...
		it('should allow a sub-node to connect to multiple root nodes', () => {
			const workflow = createWorkflow(
				[
					createNode({ name: 'AI Agent 1', type: '@resin/nodes-langchain.agent' }),
					createNode({ name: 'AI Agent 2', type: '@resin/nodes-langchain.agent' }),
					createNode({ name: 'OpenAI Model', type: '@resin/nodes-langchain.lmChatOpenAi' }),
				],
				{
					'OpenAI Model': {
			...
			const violations: ProgrammaticViolation[] = [
				{
					...
					type: 'critical',
					description:
						'Node AI Agent 2 (@resin/nodes-langchain.agent) is missing required input of type ai_languageModel',
					pointsDeducted: 50,
					metadata: {
						nodeName: 'AI Agent 2',
						nodeType: '@resin/nodes-langchain.agent',
		...
		it('should fix Vector Store missing embeddings', () => {
			const workflow = createWorkflow([
				...
				createNode({ name: 'Embeddings', type: '@resin/nodes-langchain.embeddingsOpenAi' }),
			]);
			...
			const violations: ProgrammaticViolation[] = [
				{
					...
					type: 'critical',
					description:
						'Node Vector Store (@resin/nodes-langchain.vectorStoreInMemory) is missing required input of type ai_embedding',
					pointsDeducted: 50,
					metadata: {
						nodeName: 'Vector Store',
						nodeType: '@resin/nodes-langchain.vectorStoreInMemory',
			...
			const violations: ProgrammaticViolation[] = [
				{
					...
					type: 'critical',
					description:
						'Node AI Agent (@resin/nodes-langchain.agent) is missing required input of type main',
					pointsDeducted: 50,
					metadata: {
						nodeName: 'AI Agent',
						nodeType: '@resin/nodes-langchain.agent',
