...
describe('CodeBuilderNodeSearchEngine', () => {
	...
	beforeEach(() => {
		...
		nodeTypes = [
			...
			createNodeType({
				name: '@resin/nodes-langchain.vectorStoreMemory',
	...
	describe('searchByName', () => {
		...
		it('should apply nodeFilter before ranking and limiting results', () => {
			...
			expect(results).toHaveLength(1);
			expect(results[0].name).toBe('resin-nodes-base.httpRequestTool');
		});
	...
	describe('searchByName - type name priority', () => {
		it('should return node with matching type name as first result', () => {
			...
			expect(results.length).toBeGreaterThanOrEqual(1);
			expect(results[0].name).toBe('resin-nodes-base.set');
	...
	describe('searchByConnectionType', () => {
		it('should find nodes with exact connection type match', () => {
			...
			expect(results).toHaveLength(1);
			expect(results[0].name).toBe('@resin/nodes-langchain.toolCalculator');
		...
		it('should find multiple nodes with same connection type', () => {
			...
			const anotherTool = createNodeType({
				name: '@resin/nodes-langchain.toolCode',
			...
			expect(results.map((r) => r.name)).toContain('@resin/nodes-langchain.toolCalculator');
			expect(results.map((r) => r.name)).toContain('@resin/nodes-langchain.toolCode');
		});
		...
		it('should apply name filter when provided', () => {
			...
			expect(results).toHaveLength(1);
			expect(results[0].name).toBe('@resin/nodes-langchain.lmChatOpenAi');
		});
		...
		it('should sort by score with name filter boost', () => {
			...
			expect(names).toContain('@resin/nodes-langchain.toolCalculator');
			...
			const calculatorScore = results.find(
				(r) => r.name === '@resin/nodes-langchain.toolCalculator',
			)?.score;
	...
	describe('getSubnodesForConnectionType', () => {
		it('should return default subnodes for ai_languageModel', () => {
			...
			expect(subnodes).toContain('@resin/nodes-langchain.lmChatOpenAi');
		});
		...
		it('should return default subnodes for ai_memory', () => {
			...
			expect(subnodes).toContain('@resin/nodes-langchain.memoryBufferWindow');
		});
		...
		it('should return default subnodes for ai_embedding', () => {
			...
			expect(subnodes).toContain('@resin/nodes-langchain.embeddingsOpenAi');
		});
	...
	describe('getRelatedSubnodeIds', () => {
		it('should find related subnodes based on builderHint.inputs', () => {
			...
			const relatedIds = engine.getRelatedSubnodeIds(['@resin/nodes-langchain.agent'], new Set());

			expect(relatedIds.has('@resin/nodes-langchain.lmChatOpenAi')).toBe(true);
			expect(relatedIds.has('@resin/nodes-langchain.memoryBufferWindow')).toBe(true);
		});
		...
		it('should recursively find nested subnode requirements', () => {
			...
			const vectorMemory = createNodeType({
				name: '@resin/nodes-langchain.memoryVectorStore',
				displayName: 'Vector Store Memory',
				outputs: ['ai_memory'],
				builderHint: {
					inputs: {
			...
			const relatedIds = engine.getRelatedSubnodeIds(
				['@resin/nodes-langchain.memoryVectorStore'],
				new Set(),
			);

			// Should find the in-memory vector store (direct requirement)
			expect(relatedIds.has('@resin/nodes-langchain.vectorStoreInMemory')).toBe(true);
			// Should also find OpenAI embeddings (nested requirement of vector store)
			expect(relatedIds.has('@resin/nodes-langchain.embeddingsOpenAi')).toBe(true);
		});
		...
		it('should exclude already shown node IDs', () => {
			...
			const relatedIds = engine.getRelatedSubnodeIds(
				['@resin/nodes-langchain.agent'],
				new Set(['@resin/nodes-langchain.lmChatOpenAi']),
			);

			expect(relatedIds.has('@resin/nodes-langchain.lmChatOpenAi')).toBe(false);
		});
		...
		it('should return empty set for nodes without builderHint.inputs', () => {
			...
			const relatedIds = engine.getRelatedSubnodeIds(['resin-nodes-base.httpRequest'], new Set());
	...
	describe('getNodeType', () => {
		it('should return node type by ID', () => {
			const nodeType = searchEngine.getNodeType('resin-nodes-base.code');

			expect(nodeType).toBeDefined();
			expect(nodeType!.name).toBe('resin-nodes-base.code');
