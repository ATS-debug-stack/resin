...
describe('NodeTypeParser', () => {
	describe('getNodeType with array versions', () => {
		it('should find node when version is in the array', () => {
			...
			const result = parser.getNodeType('@resin/nodes-langchain.openAi', 2.1);

			expect(result).not.toBeNull();
			expect(result?.name).toBe('@resin/nodes-langchain.openAi');
		...
		it('should find node when searching for any version in the array', () => {
			...
			const result2 = parser.getNodeType('@resin/nodes-langchain.openAi', 2);
			expect(result2).not.toBeNull();

			// Should find for version 2.1 (second in array)
			const result21 = parser.getNodeType('@resin/nodes-langchain.openAi', 2.1);
		...
		it('should return null for version not in array', () => {
			...
			const result = parser.getNodeType('@resin/nodes-langchain.openAi', 1);
