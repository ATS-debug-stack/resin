...
describe('NodeSearchTool', () => {
	...
	describe('invoke', () => {
		...
		it('should search sub-nodes by connection type', async () => {
			...
			expect(message).toContain('<node_name>@resin/nodes-langchain.toolCalculator</node_name>');
			expect(message).toContain('<node_name>@resin/nodes-langchain.toolCode</node_name>');
		...
		it('should search sub-nodes with name filter', async () => {
			...
			expect(message).toContain('<node_name>@resin/nodes-langchain.toolCalculator</node_name>');
			expect(message).not.toContain('<node_name>@resin/nodes-langchain.toolCode</node_name>');
		});
		...
		it('should handle multiple queries in a single request', async () => {
			...
			expect(message).toContain('<node_name>@resin/nodes-langchain.lmChatOpenAi</node_name>');
		});
		...
		it('should handle expression-based outputs in sub-node search', async () => {
			...
			expectToolSuccess(content, 'Found');
			expect(message).toContain('<node_name>@resin/nodes-langchain.vectorStore</node_name>');
		});
