...
const LANGUAGE_PROVIDERS: ProviderRegistry = {
	...
	anthropic: {
		build: (creds, model, fetch) => {
			const { createAnthropic } = require('@ai-sdk/anthropic') as typeof import(
				'@ai-sdk/anthropic',
			);
	...
	google: {
		build: (creds, model, fetch) => {
			const { createGoogleGenerativeAI } = require('@ai-sdk/google') as typeof import(
				'@ai-sdk/google',
			);
	...
	openrouter: {
		build: (creds, model, fetch) => {
			const { createOpenRouter } = require('@openrouter/ai-sdk-provider') as typeof import(
	...
	'aws-bedrock': {
		build: (creds, model, fetch) => {
			const { createAmazonBedrock } = require('@ai-sdk/amazon-bedrock') as typeof import(
