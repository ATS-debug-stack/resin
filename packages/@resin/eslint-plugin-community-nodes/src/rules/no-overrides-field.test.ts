...
ruleTester.run('no-overrides-field', NoOverridesFieldRule, {
	valid: [
		...
		},
		{
			...
			filename: 'package.json',
			code: '{ "name": "resin-nodes-example", "config": { "overrides": { "axios": "1.0.0" } } }',
		},
	],
	invalid: [
		{
			...
			filename: 'package.json',
			code: '{ "name": "resin-nodes-example", "overrides": { "axios": "1.0.0" } }',
			errors: [{ messageId: 'overridesForbidden' }],
		},
		{
			name: 'empty overrides object is forbidden',
			filename: 'package.json',
			code: '{ "name": "resin-nodes-example", "overrides": {} }',
			errors: [{ messageId: 'overridesForbidden' }],
		},
		{
			name: 'real-world overrides (CNOC-404 Sinch) is forbidden',
			filename: 'package.json',
			code: '{ "name": "resin-nodes-sinch", "overrides": { "axios": "1.7.0", "fast-xml-parser": "4.4.0", "minimatch": "9.0.5", "@langchain/core": "0.3.0", "qs": "6.13.0" } }',
			errors: [{ messageId: 'overridesForbidden' }],
		},
	],
});
