import { type INode, NodeConnectionTypes } from 'resin-workflow';
...
describe('rewireGraph()', () => {
	it('rewires a simple graph with a tool node', () => {
		const tool = createNodeData({ name: 'tool', type: 'resin-nodes-base.ai-tool' });
	...
	it('rewires deeply nested tools', () => {
		...
		const trigger = createNodeData({ name: 'trigger' });
		const topAgent = createNodeData({ name: 'topAgent', type: 'resin-nodes-base.ai-agent' });
		const agentTool = createNodeData({ name: 'agentTool', type: 'resin-nodes-base.ai-agent-tool' });
		const leafTool = createNodeData({ name: 'leafTool', type: 'resin-nodes-base.ai-tool' });
