...
describe('FileAccessRule', () => {
	...
	describe('detectWorkflow()', () => {
		...
		it('should detect ReadWriteFile node usage', async () => {
			const { workflow, nodesGroupedByType } = createWorkflow('wf-1', 'Test Workflow', [
				createNode('Read File', 'resin-nodes-base.readWriteFile'),
			]);
			...
			expect(result.issues[0]).toMatchObject({
				title: "File access node 'resin-nodes-base.readWriteFile' with name 'Read File' affected",
		...
		it('should detect ReadBinaryFiles node usage', async () => {
			const { workflow, nodesGroupedByType } = createWorkflow('wf-1', 'Test Workflow', [
				createNode('Read Binary', 'resin-nodes-base.readBinaryFiles'),
			]);
			...
			expect(result.issues[0]).toMatchObject({
				title:
					"File access node 'resin-nodes-base.readBinaryFiles' with name 'Read Binary' affected",
				level: 'warning',
			});
		});
			...
			expect(result.issues).toEqual(
				expect.arrayContaining([
					expect.objectContaining({
						title:
							"File access node 'resin-nodes-base.readWriteFile' with name 'Read File' affected",
					}),
					expect.objectContaining({
						title:
							"File access node 'resin-nodes-base.readBinaryFiles' with name 'Read Binary' affected",
