	...
	describe('detectWorkflow()', () => {
		it('should return no issues when no start nodes are found', async () => {
			const { workflow, nodesGroupedByType } = createWorkflow('wf-1', 'Test Workflow', [
				createNode('Manual Trigger', 'resin-nodes-base.manualTrigger'),
			]);
		...
		it('should detect disabled start node with delete message', async () => {
			const disabledStartNode = {
				...createNode('Start', 'resin-nodes-base.start'),
				disabled: true,
			};
			const { workflow, nodesGroupedByType } = createWorkflow('wf-1', 'Test Workflow', [
				disabledStartNode,
		...
		it('should detect multiple start nodes', async () => {
			const { workflow, nodesGroupedByType } = createWorkflow('wf-1', 'Test Workflow', [
				createNode('Start 1', 'resin-nodes-base.start'),
				createNode('Start 2', 'resin-nodes-base.start'),
			]);
		...
		it('should detect both enabled and disabled start nodes with appropriate messages', async () => {
			const disabledStartNode = {
				...createNode('Start Disabled', 'resin-nodes-base.start'),
				disabled: true,
			};
			const { workflow, nodesGroupedByType } = createWorkflow('wf-1', 'Test Workflow', [
				createNode('Start Enabled', 'resin-nodes-base.start'),
				disabledStartNode,
			]);
