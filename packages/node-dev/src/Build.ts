...
export async function createCustomTsconfig() {
	...
	const tsconfigPath = join(dirname(require.resolve('resin-node-dev/src')), 'tsconfig-build.json');
...
export async function buildFiles({
	...
	// Supply a node base path so that it finds n8n-core and resin-workflow
