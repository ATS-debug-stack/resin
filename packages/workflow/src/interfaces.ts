...
export interface IRelatedNode {
	/** The node type ID (e.g., '@resin/nodes-langchain.memoryBufferWindow') */
	nodeType: string;
...
export interface INodeGraphItem {
	...
	agent?: string; //@resin/nodes-langchain.agent
	is_streaming?: boolean; //@resin/nodes-langchain.agent
	prompts?: IDataObject[] | IDataObject; //ai node's prompts, cloud only
	use_responses_api?: boolean; //@resin/nodes-langchain.lmChatOpenAi
	toolSettings?: IDataObject; //various langchain tool's settings
	sql?: string; //merge node combineBySql, cloud only
	workflow_id?: string; //@resin/nodes-langchain.toolWorkflow and n8n-nodes-base.executeWorkflow
	response_mode?: string; // @resin/nodes-langchain.chatTrigger, n8n-nodes-base.webhook selected response mode
	public_chat?: boolean; // @resin/nodes-langchain.chatTrigger
	runs?: number;
	items_total?: number;
	metric_names?: string[];
	language?: string; // only for Code node: 'javascript' or 'python' or 'pythonNative'
	package_version?: string; // only for community nodes
	used_guardrails?: string[]; // only for @resin/nodes-langchain.guardrails
	mcp_client_auth_method?: string; // for @resin/nodes-langchain.mcpClientTool and @resin/nodes-langchain.mcpClient
