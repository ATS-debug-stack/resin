...
Naming is validated by `@resin/css-var-naming` in `packages/@n8n/stylelint-config/src/rules/css-var-naming.ts` for files in `@n8n/design-system/src/css`.
...
- If you must add a temporary exception, scope it tightly with `/* stylelint-disable @resin/css-var-naming */` and re-enable immediately.
