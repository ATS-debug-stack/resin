	...
	it('should call nodeTypesStore methods and update latestVerifiedVersion on mount', async () => {
		...
		renderComponent({
			props: {
				...
				activePackageName: 'resin-nodes-test',
				mode: 'update',
			},
		});
		...
		expect(nodeTypesStore.getCommunityNodeAttributes).toHaveBeenCalledWith('resin-nodes-test.test');
	});
