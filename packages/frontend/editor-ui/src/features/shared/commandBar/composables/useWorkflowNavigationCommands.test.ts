...
import * as permissionsModule from '@resin/permissions';
...
vi.mock('@resin/permissions', async (importOriginal) => ({
	...(await importOriginal()),
...
describe('useWorkflowNavigationCommands', () => {
	...
	beforeEach(() => {
		...
		Object.defineProperty(mockNodeTypesStore, 'allNodeTypes', {
			value: [{ name: 'resin-nodes-base.httpRequest', displayName: 'http request' }],
			configurable: true,
		});
			...
			async (params: { query?: string; nodeTypes?: string[]; tags?: string[] }) => {
				if (params.nodeTypes && params.nodeTypes.length > 0) {
					return [
						{
							...allWorkflows[0],
							nodes: [{ type: 'resin-nodes-base.httpRequest' }],
						} as IWorkflowDb,
					];
				}
