import { request } from '@resin/rest-api-client';
import type { JSONSchema7 } from 'json-schema';
import type { NodeParameterValueType } from 'resin-workflow';
...
export const getSchemaPreview = async (
	...
	const path = ['schemas', nodeType.replace('@resin/', ''), versionString, resource, operation]
		.filter(Boolean)
		.join('/');
