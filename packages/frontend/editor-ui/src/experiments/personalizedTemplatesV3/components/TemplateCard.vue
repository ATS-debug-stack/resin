...
import { type ITemplatesWorkflowResponse } from '@resin/rest-api-client';
...
import { N8nCard, N8nText } from '@resin/design-system';
...
const templateNodes = computed(() => {
	...
	const hubspotNode = nodeTypesStore.getNodeType('resin-nodes-base.hubspot');
	if (hubspotNode && uniqueNodeTypes.has('resin-nodes-base.hubspot')) {
		nodesToShow.push(hubspotNode);
	}

	const otherNodes = nodeTypesArray.filter((nodeType) => nodeType !== 'resin-nodes-base.hubspot');
